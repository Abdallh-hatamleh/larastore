...
<body class="relative">
    <div class="h-40 flex items-end">
        <a href="/add/c" class="bg-blue-400 rounded p-2 static top-4 left-1 z-0 mb-12 block w-fit ml-4">Add a category</a>
    </div>
    <div class="flex justify-center items-center w-screen h-screen overflow-hidden bg-black bg-opacity-80 absolute top-0 z-10 <?php if(!(isset($create) && $create === true)) echo 'invisible'?>" id="Add_overlay">
        <div class="bg-white rounded p-3 relative w-5/12 lg:w-3/12">
            <a href="/c" class="bg-slate-500 text-white rounded-full p-2 w-10 h-10 block text-center absolute -top-4 -right-4">X</a>
            <h2 class="flex justify-center">Add a category</h2>
            

<form class="max-w-md mx-auto flex flex-col" action="/c" method="post">
    @csrf
