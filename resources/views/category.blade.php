...
<div class="flex justify-center items-center w-screen h-screen overflow-hidden bg-black bg-opacity-80 absolute z-10 <?php if(!(isset($edit) && $edit === true)) echo 'hidden'?>" id="Add_overlay">
        <div class="bg-white rounded p-3 relative w-5/12 lg:w-3/12">
            <a href="/p/{{$category['id']}}" class="bg-slate-500 text-white rounded-full p-2 w-10 h-10 block text-center absolute -top-4 -right-4">X</a>
            <h2 class="flex justify-center">edit category</h2>
            

<form class="max-w-md mx-auto flex flex-col" action="/c/{{$category['id']}}" method="post">
    @csrf
    @method('put')
  <div class="relative z-0 w-full mb-5 group">
      <input type="text" value="{{$category['name']}}" name="name" id="floating_email" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
  ...
  <div class="relative z-0 w-full mb-5 group">
      <textarea type="text" name="desc" id="floating_repeat_password" class="block py-2.5 px-0 w-full text-sm text-black bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required>{{$category['desc']}}</textarea>
  ...
  <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">save</button>
    ...
    <div class="flex justify-center flex-col items-center m-auto w-6/12 my-auto h-screen">
        <a href="/c" class="flex self-start rounded bg-slate-700 text-white p-3">Go Back</a>
    <h2 class="text-lg">{{$category['name']}}</h2>
    <p>{{$category['desc']}}</p>
    <form action="/c/{{$category['id']}}" method="post" class="self-end">
        @csrf
        @method('patch')
